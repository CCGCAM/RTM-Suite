...
Python equivalent of Scripts/R/ForSPART/3-inversion_DL.R
...
for the full architecture rationale. Scoped to the TOC dataset only (not
also the TOA/Sentinel-2A one, like 2_inversion_ml.py) to keep runtime
reasonable -- swap OUT_DIR / "1_dataset_toc.csv" below for the TOA one if
you want that DL comparison too.
"""
...
def _load_predictors():
    dataset = pd.read_csv(OUT_DIR / "1_dataset_toc.csv")
...
def main():
    ...
    print(f"Using {X_full.shape[1]} predictors, {len(dataset)} samples (TOC).")
