# ==============================================================================
# Pipeline step 3: trait inversion with deep learning, via
# ToolsRTM::getMLmodel() ('CNN' or 'Hidden-layers' architectures, TensorFlow/
# Keras backend). Run Scripts/Pipeline/0-setup_python_env.R once first.
# ==============================================================================
...
# Run in a fresh R session: reticulate caches this on first use, so setting
# it after a prior failed attempt in the same session won't take effect.
...
library(reticulate)
# Must match Scripts/Pipeline/0-setup_python_env.R's py_require() call so
# reticulate resolves to the same provisioned Python environment.
...
target_trait <- "Cab"       # any LUT column works, e.g. "Car", "LAI", "Anth"
...
cat(sprintf("=== Deep-learning inversion of '%s', %d predictors, %d simulations ===\n",
