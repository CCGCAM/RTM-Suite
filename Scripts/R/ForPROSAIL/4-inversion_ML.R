# ==============================================================================
# ForPROSAIL course pipeline, step 2/3: classic ML trait inversion
...
# NN excluded: caret's nnet-based tuning grid gets very slow with this many
# predictors (~250, reflectance + indices) at only 100 training rows -- nnet's
# default MaxNWts=1000 is a poor fit for wide/short data like this (confirmed
# separately: NN trains fine on a narrower ~79-predictor dataset). Not a bug,
# just not worth the runtime here -- re-add it if you narrow the predictor set.
...
summary_plot <- ggplot(subset(df_all, status == "ok"), aes(x = algorithm, y = R2, fill = trait)) +
  ...
  labs(x = "Algorithm", y = expression(R^2~"(test)"), fill = "Trait",
       title = "ML inversion accuracy by trait and algorithm (ForPROSAIL, fourSAIL)") +
