# ==============================================================================
# ForFoursail2 course pipeline, step 1/3: simulate a LUT (foursail2 canopy
# model, the two-layer green+brown canopy variant), convolve to
# Sentinel-2A/2B/PRISMA, compute indices, save a full set of diagnostic
# figures (trait histograms, trait correlations, spectra by trait percentile,
# sensor-convolution comparison). Always 100 simulations. Feeds
# 2-inversion_ML.R and 3-inversion_DL.R next.
# ==============================================================================
...
# foursail2-specific (green/brown two-layer canopy) params. NOTE: exact-match
# `is.null(LUT[["x"]])`, not `is.null(LUT$x)` -- data.frame's `$` does partial
# name matching (e.g. `LUT$h` would silently resolve to `hspot`).
...
cat("\nSaved to '", out_dir, "/1-datasets.rds' -- read back with readRDS() in 2-inversion_ML.R / ",
    "3-inversion_DL.R ($datasets$native / $se2a / $prisma are ready for inversion).\n", sep = "")
