...
spectral.indices<-c('ARI','Greeness','TCARI','TCARI_OSAVI','GNDVI',
                    ...
                    'NDVI','MTVI2','MTVI1','EVI','GVI','Redness',
...
ggsave(paste(paths.plots,'1-Johnson-bands-',version,'.png',sep=''),
       plot=plot.Johnson,
...
version_<-'PROSAIL'
img1 <- png::readPNG(paste(paths.plots,'1-Johnson-bands-',version_,'.png',sep=''))
img2 <- png::readPNG(paste(paths.plots,'1-Johnson-indices-',version_,'.png',sep=''))


# Create a grid with two columns
plot.combined<-gridExtra::grid.arrange(
  grid::rasterGrob(img1, interpolate = TRUE),
  grid::rasterGrob(img2, interpolate = TRUE),
  ncol = 1
)

ggsave(paste(paths.plots,'2-Johnson-',version_,'.png',sep=''),
       plot=plot.combined,
       width = 20, height = 20,  dpi = 300,units = "cm")
