...
- **Tutorials, step by step** (simulate -\> sweep a trait -\> convolve onto a sensor, incl. your own sensor/camera -\> invert with ML, in R side-by-side with Python): [`Tutorials/How-in-Python.ipynb`](https://github.com/CCGCAM/RTM-Suite/blob/main/Tutorials/How-in-Python.ipynb) (R version: `How-in-R.Rmd`) -- matches the `AEO-Course/Apps/RTMs` Shiny app's own **"How in Python"** tab
