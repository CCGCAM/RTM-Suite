...
Two new R functions were added (not bugs, closing real gaps found while checking the `Apps/RTMs` Shiny app's satellite-convolution tutorial against the package's own docs): `get.spectral.convolution.srf()` (plain per-band SRF, no atmosphere — PRISMA, Sentinel-2A/B) and `get.spectral.convolution.gaussian()` (nominal center+FWHM only — EnMAP, Landsat, MODIS, and any custom sensor/camera given just band centers). Both are ported (`toolsrtm.srf.spectral_convolution_srf`/`spectral_convolution_gaussian`, see the table above) and used by the `Apps/RTMs` app instead of its own previously-private convolution helpers.
