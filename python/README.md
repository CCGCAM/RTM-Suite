...
| `toolsrtm.marmit.marmit1` / `marmit2` / `toolsrtm.get_marmit_rsoil` | `ToolsRTM/R/marmit1.R`, `marmit2.R` / `ToolsRTM/R/get.marmit.rsoil.R` | MARMIT-1 and MARMIT-2 soil reflectance models (dry-\>wet soil spectrum) -- MARMIT-2 additionally accounts for soil particle size/refractive index (`n_i`/`k_i`/`d_i`), generally more accurate for coarser soils; select via `get_marmit_rsoil(..., version='marmit2')`. Only the Bablet_2016 database is bundled (17 IDs, driest spectrum per ID only -- same scoping the R side uses), exported via `python/scratch/scratch_export_marmit.py`. |
| `toolsrtm.fluspect.fluspect_b` / `fluspect_cx` | `ToolsRTM/R/fluspect_B.R` (`getFluspect.B`) / `fluspect_Cx.R` (`getFluspect.Cx`) | FLUSPECT-B / FLUSPECT-B-Cx leaf model: PROSPECT reflectance/transmittance plus chlorophyll-fluorescence excitation-emission matrices (doubling method). A real indexing bug found in `getFluspect.Cx` during this port (see below) has since been **fixed in the R source**; `fluspect_cx` matches the fixed behavior. |
...
| `toolsrtm.indices.get_indices` | `ToolsRTM/R/getIndices.R` | \~75 VNIR + \~18 SWIR spectral vegetation indices from a reflectance spectrum (linear interpolation onto an integer-nm grid, then per-index algebra at named wavelengths). Takes `wavelengths`/`reflectance` arrays directly rather than dataframe columns matched by regex (see `indices.py` docstring) -- the numerics are unchanged. A few formulas reproduce **real bugs in the R source as-is** (not fixed): `CIgreen` references a nonexistent `r['B3']` (NaN, matching R's NA), `TCARI/OSAVI.1510` references mistyped keys (NaN), and `GnyLi`/`CI2` are each computed at two/three wavelength offsets in the R source with later assignments silently overwriting earlier ones -- only the final value survives under each name, exactly as in R. |
...
| `fluspect_cx` (refl/tran/kChlrel/kCarrel/Mb/Mf) | `ToolsRTM::getFluspect.Cx` | \~1e-15 (refl/tran), \~1e-19 (fluorescence matrices) — see fixed-bug note below |
...
| `rtmf`, native-grid (pre-spline) quantities | `SCOPEinR::get.RTMf` (fixed source) | \~1e-6 (loop-accumulated over 30 layers; tighter per-expression) |
| `rtmf`, fully-interpolated `LoF_`/`EoutF_` (211-pt display grid) | `SCOPEinR::get.RTMf` (fixed source) | \~1.5e-3 absolute worst case (documented `not-a-knot` vs `fmm` spline approximation, see below) |
| `rtmz`, `rso`/`rdo`/`Eout_` corrections (500-600nm) | `SCOPEinR::get.RTMz` (fixed source) | \~1e-5 |
| `net_radiation_lite` (`Rnuc`/`Rnhc`/`Rnus`/`Rnhs`/`Pnu_Cab`/`Pnh_Cab`) | `SCOPEinR::RTMo.R` section 4, "lite" branch (fixed source) | \~1e-13 |
| `rtmt_sb` | `SCOPEinR::get.RTMt.sb`, "lite" scalar-per-layer branch | \~1e-13 |
| `ebal` (converged temperatures) | `SCOPEinR::get.ebal` (fixed source) | \~1e-3 (energy-balance totals \~1-2%, see module docstring for why) |
| `get_scope`, exact-formula outputs (`LAIsunlit`/`Pnsun_Cab`/`Pnsha_Cab`/`Pntot_Cab`/TOC `refl`) | `SCOPEinR::get.SCOPE` (fixed source, package's own bundled example LUT) | \~1e-9 to \~1e-13 |
| `get_scope`, iterative-convergence outputs (temperatures, energy-balance totals, `Ja`/`PNPQ`/`fqe`) | `SCOPEinR::get.SCOPE` (fixed source) | \~1-2% (\~5% worst case, `fqe`) -- same per-layer-loop biochemistry divergence as `ebal` itself |

The `get_indices` reference was generated with `devtools::load_all()` against current `ToolsRTM/R/` source, not `library(ToolsRTM)` against the installed package -- the installed build still had `getIndices()`'s pre-fix behaviour (returned a bare matrix, never merged the original data back) from before this repo's own bug fixes to that function, so `scratch_export.R` now loads source everywhere for exactly this reason.

No unresolved numerical discrepancies were found — every ported function matches R to floating-point precision. (Two spots worth knowing about, not discrepancies: (1) `run_rtmo`'s `rso`/`refl` legitimately produce `NaN`/`inf` at far-thermal wavelengths where `Esun_` underflows to \~0 — this reproduces R's own behaviour at the same wavelengths, not a bug; (2) `soilwat`'s use of `scipy.stats.poisson.pmf(round(mu), k)` looks backwards at first glance vs. the natural `pmf(k, mu)` — this mirrors a quirk of the original R call `stats::dpois(round(mu), k)` exactly, and is covered by the BSM regression test.)

## R bugs fixed during this port

Porting each function against a real R call occasionally surfaced genuine bugs in the R source itself (not porting differences) — fixed at the source in R, with the Python port matching the fixed behavior. Kept here as a factual log, not a narrative:

| R function | Bug |
|------------------------------------|------------------------------------|
| `getFluspect.Cx` | `Iwlf <- intersect(wlp, wlf)` used matched *values* as array *positions*, reading Kubelka-Munk coefficients at the wrong wavelengths (1039-1249nm instead of 640-850nm) — wrong `Mb`/`Mf` fluorescence matrices on every call. Fixed to `which(wlp >= min(wlf) & wlp <= max(wlf))`. |
| `get.RTMf` | 18-expression `vector * matrix` column-recycling bug (`wfEs` through `Femplu`) fixed with `sweep()`; separate `absfs_nl <- c(absfsfo)` copy-paste (should be `c(absfs)`). |
| `get.RTMz` | Same recycling bug as `get.RTMf`; its real output was only ever computed inside an unreachable `if (get.plots == TRUE)` block, so normal calls silently returned the input unmodified; plus a `Po[1:nl+1]` indexing bug, a hardcoded `dim = c(30, 13, 36)`, and a MATLAB `sum(A,2)`-to-R mistranslation (`sum()` has no such argument) fixed to `rowSums()`. |
| `RTMo.R` (lite section 4) | `Rndir <- fs_ * Asun[j]` reused a stray loop variable `j` (always the last canopy layer) instead of the full per-layer vector — net radiation came out constant across layers. |
| `get.zo_and_d` | Degenerate-canopy branch referenced an unassigned `d`; fixed to the evidently-intended `d <- 0`. |
| `SPART` | `get.plots` handling used `missing()` instead of the argument's actual value, so an explicit `get.plots = FALSE` was silently overridden to `TRUE` — every call was plotting internally regardless of what was passed. |
| `Compute_BRF` | Same `missing()`-vs-value bug for `short.waves`, causing `Es`/`Ed` to silently recycle (wrap around) for Fluspect-based leaf models' shorter (2001 vs 2101) output length. |
| `foursail2` | Never truncated `rsoil` to the Fluspect models' 2001-length domain (unlike `foursail()`), silently recycling a full-length soil spectrum through the whole SAIL2 core. |
| `getFluspect.B` / `getFluspect.Cx` | `fqe = 0` (a valid input) fell into a `if (fqe_ > 0) {...}` block with no `else` that also held the function's only `return()` — silently returned `NULL`, propagating into a zero-length `rdot`/`rsot`. Fixed by adding an `else` branch (all-zero Mb/Mf, the physically correct answer) and moving `return()` outside the `if`/`else`. |
| `get.spectral.convolution.rfl()` | Its own documented `@examples` crashed: `sensor.i` is documented as a string but the code needed the bundled sensor *object*. Separately, 6 of 9 bundled sensors (Landsat 4/5/7, Sentinel-3A/B OLCI, Terra/Aqua MODIS) had mislabeled band-center wavelengths from assuming `id_smac_in_all` was always the identity permutation. |
| `carspls()` / `get.cars.pls()` | Called bare `ceil()` without importing it from `pracma` — crashed on every call, including their own documented example. Fixed to base R's `ceiling()` (numerically identical, no import needed). |

Two new R functions were added (not bugs, closing real gaps found while checking the AEO-Course app's satellite-convolution tutorial against the package's own docs): `get.spectral.convolution.srf()` (plain per-band SRF, no atmosphere — PRISMA, Sentinel-2A/B) and `get.spectral.convolution.gaussian()` (nominal center+FWHM only — EnMAP, Landsat, MODIS, and any custom sensor/camera given just band centers). Both are ported (`toolsrtm.srf.spectral_convolution_srf`/`spectral_convolution_gaussian`, see the table above) and used by the AEO-Course app instead of its own previously-private convolution helpers.
