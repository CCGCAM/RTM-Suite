---
title: "09. Does SIF Add Information About Photosynthesis?"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{09. Does SIF Add Information About Photosynthesis?}
...
```{r predictors-plot, fig.width=10, fig.height=4.5}
op <- par(mfrow = c(1, 2))
...
modest, since `getLUT.SCOPE()`'s fully independent trait sampling (unlike
Tutorial 05's deliberately-correlated Cab~Vcmax25 LUT) is a
deliberately hard, information-poor setup.
...
SIF, a Cab~Vcmax25-correlated LUT, and gets to R²=0.356 on the exact same
`Actot` target. Section 5 below adds one more predictor to this page's
own ablation to show that transition starting to happen, without
...
`Cab`, and correlates `Cab`~`Vcmax25`) -- just one more predictor added to
this page's own NDVI+SIF ablation, to see the R² actually start moving:
...
is exactly the pattern Tutorial 11 pushes much further -- ten real
sensor bands (not one ratio index), a properly correlated LUT, and SIF
back in the mix -- to reach R²=0.356.
...
- **Tutorial 11** -- the capstone: `Actot` retrieval extended to a real
  Sentinel-2 time series, where SIF isn't available as a predictor at all.
