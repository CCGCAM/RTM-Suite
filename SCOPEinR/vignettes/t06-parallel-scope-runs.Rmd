---
title: "06. Large-Scale and Parallel SCOPE Runs"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{06. Large-Scale and Parallel SCOPE Runs}
...
`get.SCOPE()` is far more expensive per call than `foursail()` (ToolsRTM
Tutorial 06) -- dominated by the iterative energy-balance solve
...
difference is the whole reason `get.SCOPE.parallel()` exists as a
...
For a real production run, `get.SCOPE.outputs()` saves everything to
disk rather than keeping it all in memory:
...
run's nested output list and saved as flat columns -- reflectance,
LIDF angles, the LIDFb parameter, the Ft/Fo fluorescence-efficiency
ratio, and the `rdo` BRDF component (Tutorial 02) in this example.
