---
title: "10. End-to-End SCOPE Pipeline"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{10. End-to-End SCOPE Pipeline}
...
parallel runs, sensitivity, hybrid inversion, SIF-vs-photosynthesis. This
page runs the whole chain together -- simulate, convolve, index, invert
-- and, unlike the optical-only ToolsRTM models, `get.SCOPE()`'s
`leaf.model`/`canopy.model` arguments are **not functional**: SCOPE
always runs its own integral multi-layer Fluspect-Cx + RTMo. There is one
leaf/canopy configuration here, not a choice to expose.
...
```{r corr-heatmap, fig.width=8, fig.height=8}
...
LUT, exactly because `getLUT.SCOPE()` samples it independently. That
single fact in the heatmap **is** why Section 5 below finds `Vcmax25`
unretrievable, and why Section 6 fixes it by imposing a correlation the
LUT doesn't have on its own.

## 5. Does SIF help retrieve `Vcmax25`? Reflectance+indices alone vs. with SIF added

```{r invert-vcmax, results='hide'}
set.seed(1)
...
Section 4's heatmap already showing why. The root cause: `getLUT.SCOPE()`
samples `Vcmax25` fully independently of `Cab`/leaf nitrogen/everything
else. In real leaves, Vcmax correlates with nitrogen and chlorophyll
content -- and that correlation is exactly what real SIF-Vcmax
remote-sensing studies rely on as their proxy signal. Sampled
independently here, there's no such signal for *any* method to find.

## 6. A fair test: correlate `Vcmax25` with `Cab` first

```{r fair-test, results='hide'}
...
```{r fair-test-plot, fig.width=6, fig.height=6}
...
direct radiative-transfer path. This is the actual, correct interpretation
of "SIF as a photosynthesis proxy" in the literature: it works to the
extent traits correlate with photosynthetic capacity in the real world,
not because SIF has some unique direct window into `Vcmax25` that
reflectance-linked correlations don't already partly provide.
