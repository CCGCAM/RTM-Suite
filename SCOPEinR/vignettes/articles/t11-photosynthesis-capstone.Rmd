---
title: "11. Capstone: ML Inversion of Net Photosynthesis, Applied to Real Sentinel-2"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{11. Capstone: ML Inversion of Net Photosynthesis, Applied to Real Sentinel-2}
...
Tutorial 08's fully independent trait sampling left `Vcmax25`
(and therefore `Actot`) essentially unretrievable -- Tutorial 10 traced
this to `getLUT.SCOPE()` not correlating `Vcmax25` with anything else,
unlike real leaves. This page uses that same fix from the start:
...
```{r two-models-plot, fig.width=10, fig.height=4.5}
op <- par(mfrow = c(1, 2))
...
for a specific flux. `rf_withsif` is kept only for this comparison and is
...
composites, wrapped in `tryCatch()` per date since any individual window
...
Sections 4-5 reduced every date to one site-mean NDVI/`Actot` value. This
section reuses one of those exact same already-retrieved cubes (no new
STAC call) to show the actual 2D image and a real per-pixel retrieval --
the same spatial pattern verified in ToolsRTM's own real-EO tutorials
(`15-real-eo-application.Rmd`'s NDVI/LAI maps, `17-forest-time-series.Rmd`'s
STAC retrieval), applied here to `Actot` instead of a structural trait.
...
The three natural-color bands (red = B04, green = B03, blue = B02) first,
before anything derived from them -- so the NDVI and `Actot` maps in 6b/6c
below can be read against what the forest actually looked like from
orbit that month, not just as abstract colored grids.

```{r rgb-quicklook, dpi = 200, fig.width = 6, fig.height = 6}
...
```{r ndvi-map, dpi = 200, fig.width = 6, fig.height = 6}
...
-- the same building block ToolsRTM's `getSpatialTrait()` uses
internally, done here explicitly:

```{r actot-map, dpi = 200, fig.width = 6, fig.height = 6}
...
4-5 tracked through time, but where within that same scene photosynthesis
is predicted to be higher or lower.
...
over: (1) the training LUT's meteorology (`Ta`, `Rin`, `Rli`, ...) is
held at `inputs_SCOPE.csv`'s own defaults, not the real meteorological
...
instead of a flux) -- together, one hand-written trait row through to
two real satellite-derived retrievals, across both packages this suite
is built from.
