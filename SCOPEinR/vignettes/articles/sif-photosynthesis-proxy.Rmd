---
title: "Does SIF add information about photosynthesis beyond greenness?"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Does SIF add information about photosynthesis beyond greenness?}
...
result worth taking away: even a weak, individually-unreliable SIF signal
adds *some* real information on top of NDVI that NDVI alone doesn't
...
isolates `Vcmax25` from the other nuisance traits (as `How-in-Python-SCOPEinR.ipynb`'s
small perturbation-only LUT does), would be expected to show the effect
more clearly than this deliberately full-range, information-poor setup.
...
a genuinely different signal from greenness rather than a noisier version
of the same thing.
...
- `sensitivity-scope`: establishes that `Vcmax25` has essentially no
  ...
  consequence of.
- `inversion-scope`: retrieves individual traits (`Cab`, `LAI`, `Vcmax25`)
  from reflectance alone; this page asks a related but different
  question -- not "can I retrieve trait X", but "does adding SIF as a
  predictor help retrieve a *flux* (`Actot`) that reflectance alone
  struggles with".
