---
title: "07. Sensitivity: Direct vs. Indirect Trait Effects"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{07. Sensitivity: Direct vs. Indirect Trait Effects}
...
This page sweeps one trait at a time and looks at three outputs:
...
`Actot` responds strongly and directly to `Vcmax25`, as expected from the
Farquhar-type model. The reflectance panel is the real point: any spread
there is *not* Fluspect/4SAIL reacting to `Vcmax25` (it has no such
input) -- it's the small, indirect effect of a different leaf temperature
feeding into the thermal part of the spectrum. Compare its magnitude to
the `Cab` sweep above.
...
- **Tutorial 08** -- hybrid inversion: retrieving Cab, LAI, and Vcmax25
  from reflectance, and seeing which one this sensitivity result predicts
  will fail.
