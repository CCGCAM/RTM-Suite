...
`SCOPEinR` is an R port of **SCOPE** (Soil Canopy Observation, Photochemistry
and Energy fluxes), a model published and maintained by Christiaan van der
Tol, Wouter Verhoef, Peiqi Yang, Egor Prikaziuk and collaborators. SCOPE's
...
matches the Python port `scopeinpython`'s own license. `SCOPEinR` depends
on [`ToolsRTM`](https://gitlab.com/caminoccg/toolsrtm) for leaf-level optics
(PROSPECT-D/-PRO) -- see that package's own
...
`SCOPEinR` developers -- always cite the original publication(s) above when
using this package in scientific work, in addition to citing
RTM-Suite/SCOPEinR.
