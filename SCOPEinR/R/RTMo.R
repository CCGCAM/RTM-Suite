...
getRTMo<-function(data.spectral,atmo,
  ...
  TOCirr <- get.calcTOCirr(atmo, data.meteo, rdd, rsd, wl, nwl = 2162,
                            spectral = data.spectral, soil = data.soil)
  ...
  if (options.calc_vert_profiles$Value == 1  ) {
    ...
    if (options.lite$Value != 1){
      ...
      # Lite mode: Pnuc/Pnuc_Cab/Pnuc_Car are already [nl] (no leaf-angle
      # dimension to reduce via meanleaf() here), so the per-layer profile
      # is the direct sunlit/shaded mix, same weighting as the non-lite
      # branch above.
      profiles <- list()
      profiles[['Pn1d']] <- ((1-Ps[1:nl]) * Pnhc + Ps[1:nl] * Pnuc)
      profiles[['Pn1d_Cab']] <- ((1-Ps[1:nl]) * Pnhc_Cab + Ps[1:nl] * Pnuc_Cab)
      profiles[['Pn1d_Car']] <- ((1-Ps[1:nl]) * Pnhc_Car + Ps[1:nl] * Pnuc_Car)
