...
getLUT_time<-function(inputs=NULL, dataICOS= NULL, timeStart='2018-01-01', timeEnd='2018-01-30', freq.hour = 'diurnal', nLUT=100,SetFixedSeed=T){
  ...
  lubridate::minute(data.ICOS.t$DateS) <- 0
  ...
  for (i in c(1:length(time))){
    pb$tick()
      ...
      if (SetFixedSeed == T | is.null(SetFixedSeed)){
           list.time[[i]]<-data.frame(t=time[i], ToolsRTM::getLUT(inputs = inputs, nLUT = nLUT, setseed = 123))
         }  else{
          list.time[[i]]<-data.frame(t=time[i], ToolsRTM::getLUT(inputs = inputs, nLUT = nLUT, setseed = i*123))
         }
