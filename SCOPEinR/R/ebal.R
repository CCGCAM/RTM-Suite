...
get.ebal <- function(data.rad, data.gap, data.meteo, data.soil, data.canopy, data.leafbio,data.leafopt,
  ...
  # Arithmetic with Rnuc itself carries over its real shape (a plain [nl]
  # vector in lite mode, [13,36,nl] otherwise), matching the same idiom
  # used for Ccu below (Ca + 0*Rnuc).
  Tcu <- Ta + 0.3 + 0 * Rnuc # leaf temperature (sunlit leaves)
  ...
  while (CONT) {
    ...
    if (options.Fluorescence_model$Value == 2) {
        # Value 2: Magnani 2012 model (get.biochemical.MD12(), authored by
        # Federico Magnani) -- matches the option's own documented mapping
        # (0: empirical/Flexas, 1: sigmoid for Kn, 2: Magnani 2012).
        ...
        # Value 0 (empirical/Flexas) and Value 1 (sigmoid for Kn) both land
        # here: get.biochemical() implements the Value 0 model only -- a
        # separate Value 1 (sigmoid Kn) implementation does not currently
        # exist in this port, so Value 1 falls back to the Value 0 model
