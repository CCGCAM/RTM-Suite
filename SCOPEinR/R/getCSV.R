...
getCSV <- function(path.out=NULL, n.folders=4, files.names='All') {
  ...
  if (!dir.exists(folder_out)) {
    ...
    # names_pattern[k] is already a full filename ending in ".csv" (e.g.
    # "refl.csv") -- get.SCOPE.plots()'s exact-name setdiff() filter
    # depends on that name matching exactly, with no extension appended.
