...
get.RTMz <- function(data.spectral, data.rad, data.soil, data.leafopt, data.canopy, data.gap,
 ...
 for (k in 1:2){
   ...
   ################################################################
   ## The layer recursion (Y/U), the outgoing-radiance terms (piLo1..4)
   ## and LoF_[,k] run inside this k-loop so they're computed
   ## unconditionally, for both k=1 (sun) and k=2 (sky).
   ################################################################
