...
#' @param nwl integer. Number of wavelengths in the spectral domain.
#' @param spectral list. Spectral-domain object (as built elsewhere in this
#'   package, e.g. \code{data.spectral} in \code{getRTMo()}) -- used here
#'   for its \code{reg1}/\code{reg2}/\code{reg3}/\code{IwlT} index ranges.
#' @param soil list. Soil properties object (e.g. \code{data.soil} in
#'   \code{getRTMo()}) -- used here for its \code{rs_thermal} value.
#'
#' @return A list with:
#' \describe{
...
#' @examples
#' \dontrun{
#' TOCirr <- get.calcTOCirr(atmo, meteo, rdd, rsd, wl, nwl, spectral, soil)
#' }
get.calcTOCirr <- function(atmo, meteo, rdd, rsd, wl, nwl, spectral, soil){
  ...
  if(!"Esun_" %in% names(atmo)){
    ...
    # atmo$WL (nm) is a MODTRAN wavenumber-derived grid (fractional nm
    # values, not aligned to SCOPE's own integer-nm wl grid), so an exact
    # match (atmo$WN/WL %in% wl) selects zero or near-zero rows. Interpolate
    # each needed transmittance/radiance column onto wl instead.
    t1  <- stats::approx(x = atmo$WL, y = atmo$T1,  xout = wl, rule = 2)$y
    t3  <- stats::approx(x = atmo$WL, y = atmo$T3,  xout = wl, rule = 2)$y
    t4  <- stats::approx(x = atmo$WL, y = atmo$T4,  xout = wl, rule = 2)$y
    t5  <- stats::approx(x = atmo$WL, y = atmo$T5,  xout = wl, rule = 2)$y
    t12 <- stats::approx(x = atmo$WL, y = atmo$T12, xout = wl, rule = 2)$y
    t16 <- stats::approx(x = atmo$WL, y = atmo$T16, xout = wl, rule = 2)$y
    ...
    # rdd/rsd are only padded here if they don't already span the full
    # nwl-length domain (some callers pass an optical-only, e.g. 2001-long,
    # spectrum that needs the thermal region appended; get.calcTOCirr's own
    # caller in getRTMo() already passes full nwl-length rdd/rsd, so no
    # padding is added in that case).
    pad_n <- nwl - length(rdd)
    rdd_wl <- if (pad_n > 0) c(c(rdd), rep(soil[['rs_thermal']], pad_n)) else c(rdd)
    rsd_wl <- if (pad_n > 0) c(c(rsd), rep(soil[['rs_thermal']], pad_n)) else c(rsd)
