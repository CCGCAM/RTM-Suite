...
#' @param canopy.model Not currently functional -- see \code{\link{get.SCOPE}} (same underlying limitation: SCOPE's canopy engine isn't swappable yet). Passing anything other than \code{'fourSAIL'} triggers a warning.
#' @param leaf.model Not currently functional -- see \code{\link{get.SCOPE}}. Passing anything other than \code{'fluspect-CX'} triggers a warning.
...
get.SCOPE.ind<-function(LUT,options.SCOPE=data.opts,optipar,path.out,
  ...
  ##################################################################################
  # See get.SCOPE.R for the full explanation: SCOPE's own leaf/canopy engines
  # aren't swappable for ToolsRTM's standalone models without reimplementing
  # them inside SCOPE's multi-layer/gap-probability framework.
  if (!missing(canopy.model) && !identical(canopy.model, 'fourSAIL')) {
    warning("get.SCOPE.ind(): canopy.model = '", canopy.model, "' was requested but is a ",
            "no-op -- see get.SCOPE.R for details.", call. = FALSE)
  }
  if (!missing(leaf.model) && !(leaf.model %in% c('fluspect-CX', 'fluspect-Cx'))) {
    warning("get.SCOPE.ind(): leaf.model = '", leaf.model, "' was requested but is a ",
            "no-op -- see get.SCOPE.R for details.", call. = FALSE)
  }
  ...
  data.leafbio<- SCOPEinR::getinputLUT(inputLUT=LUT[1,], dataset='leafbio')
  data.canopy<- SCOPEinR::getinputLUT(inputLUT=LUT[1,], dataset='canopy')
  data.meteo<- SCOPEinR::getinputLUT(inputLUT=LUT[1,], dataset='meteo')
  ...
  data.timeseries<- SCOPEinR::getinputLUT(inputLUT=LUT[1,], dataset='timeseries') ## this is xyt in SCOPE
  data.angles<- SCOPEinR::getinputLUT(inputLUT=LUT[1,], dataset='angles')
  data.mly<- SCOPEinR::getinputLUT(inputLUT=LUT[1,], dataset='mly')
  ...
  # ALWAYS restore it, however this function exits (including on error) --
  # previously this was never restored, silencing every R warning (not just
  # this function's) for the rest of the caller's session.
  ...
  data.timeseries<-SCOPEinR::getinputLUT(inputLUT=LUT[1,], dataset='timeseries')
