...
rnormcorV <- function(x, rho, ...){
  ...
  #   y2 <- pnorm(y)
  #   fit <- MASS::fitdistr(y, densfun="normal")
...
rchisqcor <- function(x, rho, na.rm = FALSE) {
  ...
  fit <- MASS::fitdistr(y, densfun="chi-squared", list(df=2, ncp=1), 
...
rpoiscor <- function(x, rho, na.rm = FALSE, ...){
  ...
  fit <- MASS::fitdistr(y, densfun="Poisson")
...
##' @references modified from Eric Neuwirth, R-help mailing list post "Generating uniformly distributed correlated data" (2011)
##' @examples
##' x <- runif(1000)
...
rweibullcor <- function(x, rho, na.rm=FALSE) {
  ...
  fit <- try(MASS::fitdistr(y2, densfun="weibull"))
...
rgammacor <- function(x, rho, na.rm = FALSE){
  ...
  fit <- MASS::fitdistr(y2, densfun="gamma")
...
rnegbinomcor <- function(x, rho, scale=NULL, na.rm = FALSE){
  ...
  fit <- MASS::fitdistr(ceiling(y2*scale), densfun="negative binomial")
