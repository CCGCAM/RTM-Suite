...
getFluspect.B<-function(inputsLeaf,inputsOptipar, version = 'PROSPECT-D' )  {
  ...
  #
  # NB (fix): this block used to end with `return(LRT)` *inside* the
  # `if (fqe_ > 0)` -- refl/tran (computed above, unconditionally, at lines
  # 174-175) never depend on fqe at all, but wrapping the function's only
  # return statement inside this fluorescence-only conditional meant that
  # fqe_ == 0 (a valid, physically meaningful "no fluorescence" input --
  # exactly the value this app's RTM tab's own fqe slider defaults its
  # sweep range to start at) made the WHOLE function return NULL instead of
  # a proper reflectance/transmittance result: `if (FALSE) {...}` with no
  # `else` evaluates to NULL, which was then the last expression in the
  # function body. That NULL propagated into foursail()'s `rho <- LRT[[2]]`
  # (silently becoming NULL, not an error) and from there into
  # foursail_core(), which returned a *zero-length* rdot/rsot instead of
  # erroring -- confirmed via direct repro (fqe=0 -> rdot len 0; fqe=0.01+
  # -> rdot len 2001) and via the AEO-Course PROSAIL app's Sensitivity
  # sub-tab, whose fqe sweep necessarily includes 0 at one end of its range
  # and crashed downstream with "replacement has 1 row, data has 0" trying
  # to use that empty result. Fixed by computing fallback all-zero Mb/Mf
  # matrices (fqe=0 means no fluorescence at all, so this is not just a
  # crash-avoidance shortcut but the physically correct answer) in an
  # `else` branch, and moving `LRT`/`return(LRT)` below, so the function
  # always returns refl/tran regardless of fqe.
  if (fqe_ > 0) {
   ...
   #  }
    ...
    # fqe_ <= 0: no fluorescence -- zero-valued Mb/Mf matrices, same (wlF x
    # wlE) shape the doubling routine above would otherwise produce.
    spectral <- define_bands()
    nwlf <- length(spectral[['wlF']])
    nwle <- length(spectral[['wlE']])
    g1n <- g2n <- f1n <- f2n <- matrix(0, nrow = nwlf, ncol = nwle)
  }
