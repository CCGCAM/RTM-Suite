...
getFluspect.B<-function(inputsLeaf,inputsOptipar, version = 'PROSPECT-D' )  {
  ...
  #
  # refl/tran (computed above, unconditionally, at lines 174-175) don't
  # depend on fqe. fqe_ == 0 is a valid, physically meaningful "no
  # fluorescence" input -- the else branch below computes all-zero Mb/Mf
  # (the physically correct answer for zero fluorescence quantum yield),
  # and LRT/return(LRT) sits below both branches so the function always
  # returns refl/tran regardless of fqe.
  if (fqe_ > 0) {
