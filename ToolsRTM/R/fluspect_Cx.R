...
getFluspect.Cx<-function(inputsLeaf,inputsOptipar, version = 'Cx' )  {
  ...
  #
  # NB (fix): same bug as ToolsRTM::getFluspect.B (see that function's own
  # comment for the full detail) -- `return(LRT)` used to sit *inside* this
  # `if (fqe_ > 0)`, so fqe_ == 0 made the whole function return NULL
  # instead of refl/tran (which don't depend on fqe at all, computed
  # unconditionally above), silently propagating into a zero-length
  # rdot/rsot downstream instead of erroring. Fixed the same way: an `else`
  # branch with all-zero Mb/Mf (physically correct for zero fluorescence
  # quantum yield), and LRT/return(LRT) moved below so it always fires.
  if (fqe_ > 0) {
