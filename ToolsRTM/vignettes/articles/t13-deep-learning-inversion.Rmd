...
An earlier version of this page passed `getMLmodel()` a `dataset`
...
monitoring validation loss) never actually triggered -- validation loss
was still falling at epoch 150, meaning this network hadn't finished
converging within the epoch budget this page's build time allows. That
by itself is a real, honest limitation of this demo (a production run
would keep training, or raise the learning rate), not a claim that 150
epochs is generally sufficient.
...
**On this LUT (6000 rows, 10 Sentinel-2 bands), Random Forest clearly
beats the dense network, not the other way around.** This is the real
result, not a hypothetical -- and it's exactly the outcome Section 9
...
run_trait <- function(trait) {
  ...
  dl <- getMLmodel(dataset = df[train_idx, ], depVar = trait, model = "Hidden-layers",
                    optimizer = "adam", n.epochs = 300, batch.size = 64, save.model = FALSE,
                    depVar.trans = TRUE)
  pred_dl_scaled <- as.numeric(predict(dl$model, as.matrix(df[test_idx, feat])))
  ...
  data.frame(trait = trait, n_features = length(feat),
             R2_dense = r2_f(LUT[test_idx, trait], pred_dl),
             R2_RF = r2_f(LUT[test_idx, trait], pred_rf))
}
feature_results <- do.call(rbind, lapply(c("Cab", "LAI"), run_trait))
```
...
**For Random Forest, this is at worst free and at best a real win.**
On Cab, 9 well-chosen features (5 core bands + 4 correlated indices)
essentially match Section 5's RF fit on all 10 raw bands (0.853 here
vs. 0.858 there) -- RF was already near its ceiling on Cab either way.
The real payoff is LAI, never even attempted with the generic 10-band
set above: 0.685, a strong result from a fraction of the inputs.
Fewer, better-chosen features cost RF nothing on the trait it already
handled well, and open up a second trait it hadn't been asked about
at all -- consistent with Tutorial 12's finding, extended here to a
trait Tutorial 12 didn't cover as its headline case.

**For the dense network, this same feature set is actively harmful,
and it stayed harmful after two standard fixes.** Both `R2_dense`
values above are catastrophically negative -- not just "worse than
RF," but far worse than predicting the mean every time. Section 1's
relu-output fix (this page's original bug) does not explain it: that
fix already applies here too. Adding `depVar.trans = TRUE` (also
range-scaling the target, not just the inputs -- the standard second
thing to check for an unstable regression network) made it *worse*,
not better, for both traits. With RF fitting the same 9 columns
essentially perfectly, the problem isn't the features -- it's that
`getMLmodel()`'s dense architecture, as configured on this page, does
not train stably on a small, highly informative, and (indices being
built from overlapping bands) fairly collinear input. That's a real,
documented limitation of this specific setup, not a result to quietly
drop: it's consistent with Section 9's broader point below that RF is
the forgiving default, and dense/CNN need real care -- learning-rate
tuning, regularization, or architecture changes beyond this page's
scope -- to earn their keep on inputs like this one.
...
| `get.inversion()` (Tutorial 12) | Yes | The most forgiving of the three in practice -- RF in particular stayed usable even on this page's 150-row original LUT, and clearly beat both DL architectures above on a 6000-row one | General-purpose retrieval; often the right default |
| `getMLmodel()` (this page) | Yes, more so | Needs the most: on a 6000-row LUT / 10 multispectral bands, neither dense nor CNN beat Tutorial 12's RF here (Sections 5-6) | Large LUTs, hyperspectral input, and/or more complex nonlinear mappings than RF captures well -- not a default choice |

## 9. When should I use deep learning?

Not "when it's available" -- this page's own results argue against
that. A DL model is more likely to earn its extra complexity when
**several** of these hold at once, not just one:

- **LUT size**: thousands to tens of thousands of rows, not hundreds --
  Section 5 used 6000 and RF still won comfortably (Section 7 checks
  whether a smaller, better-chosen feature set changes that).
...
`get.inversion()` (Tutorial 12) -- especially Random Forest -- is the
sensible default. Reach for `getMLmodel()` when the LUT is genuinely
