---
title: "02. From Leaf to Canopy Reflectance"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{02. From Leaf to Canopy Reflectance}
...
both boxes: every leaf model this package implements, every canopy model,
and how a trait change at the leaf level propagates to a change at the
canopy level. For what every trait below actually means, its unit, and
its realistic range, see the [Parameter & Trait
Glossary](parameter-glossary.html) -- this page is about the mechanism,
that page is the reference to come back to.
...
|---|---|---|---|
| `prospect_PRO()` | `prospect_PRO(N, Cab, Car, Anth, Cbrown, EWT, LMA, alpha, Prot, CBC)` | 400-2500nm | Splits dry matter into `Prot` (protein) + `CBC` (carbon-based constituents) instead of one `LMA` |
...
reflectance/transmittance and turn it into a canopy-level BRF, but differ
in what canopy structure they represent -- and, correspondingly, in what
extra parameters the LUT needs:

| Model | Canopy representation | Extra parameters (beyond LAI/LIDFa/hotspot/geometry) | `foursail()`-family call | Return |
|---|---|---|---|---|
| `foursail()` | Single-layer turbid medium (classic PROSAIL) | none | `foursail(inputLUT, rsoil, LeafModel)` | `list(rdot=, rsot=, ...)` |
| `foursail2()` | Two-layer canopy (green + brown/senescent) | `fraction_brown`, `diss`, `Cv`, `Zeta` | `foursail2(inputLUT, rsoil, LeafModel)` | `list(rdot=, rsot=, ...)` |
| `inform()` | Forest: explicit tree crowns over an understory + background | `LAIu`, `sd` (stem density), `cd` (crown diameter), `h` (tree height), `skyl` | `inform(inputLUT, rsoil, LeafModel)` | `rsot` directly (a plain numeric vector, not a list) |

`inform()`'s return shape (a bare vector, not a `list(rdot=, rsot=)`) is a
real, useful-to-know inconsistency across the three canopy functions --
`simulate_RTM()` (used in later tutorials) normalizes this away, but the
raw functions used directly here do not.
...
Sweep `Cab` and `EWT` one at a time (everything else fixed) and watch the
canopy-level spectrum respond -- the leaf-to-canopy chain preserves each
trait's own spectral signature, just attenuated/reshaped by the canopy:

```{r sweep, fig.height = 7}
...
in the visible/red-edge (chlorophyll absorption), `EWT` in the SWIR (water
absorption), `LAI` mainly in the NIR plateau height (more leaf layers,
more multiple scattering) while the visible saturates quickly once the
canopy is closed enough to hide the soil. Tutorial 10 formalizes this with
proper global sensitivity analysis (Sobol/Johnson indices) rather than
