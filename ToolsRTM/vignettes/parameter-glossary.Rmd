---
title: "Parameter & Trait Glossary"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Parameter & Trait Glossary}
...
it differs from its siblings. Section 7 below cross-references every
...
SCOPE builds a fully independent, larger model on top of this same
leaf-optics/canopy-BRDF idea -- energy balance, photosynthesis and
fluorescence coupled together, not just reflectance -- covered in its own
[Trait & LUT Glossary](../../scopeinr/articles/trait-glossary.html).

## 2. Leaf traits

All five leaf models (`prospect_D` -- bundled inside `foursail(...,
LeafModel = "PROSPECT-D")`, `prospect_PRO()`, `liberty()`,
`getFluspect.B()`, `getFluspect.Cx()`) build on the same PROSPECT
physics: a leaf is treated as a stack of absorbing/scattering plates,
and each trait below is one absorbing constituent (a pigment, water,
dry matter) or one structural parameter of that stack.
...
supplying both `LMA` and non-zero `Prot`/`CBC` in one `foursail()` LUT
row is a modelling choice, not something the package validates for you
(`getMLmodel()`'s R version silently keeps whichever the leaf-model
branch you called actually reads).

### 2.1 LIBERTY-only structural traits

`liberty()` targets conifer needles, not broadleaves, and needs a
different structural parameterization -- no `N`/`alpha` Fresnel-optics
layer, but explicit cell geometry instead:
...
Once a leaf model produces reflectance/transmittance, `foursail()`,
`foursail2()`, and `inform()` turn it into a canopy-level BRF. All three
share the leaf-angle-distribution and geometry parameters below;
`foursail2()` and `inform()` each add their own extra layer of structure.
...
`inform()`'s canopy-level `LAI` is the **overstorey** (tree-crown) LAI
only -- `LAIu` is added as a separate, independently-varying understorey
term, not a component subtracted from `LAI`.

## 4. Named leaf-angle distributions

`LIDFa`/`LIDFb` rarely need to be hand-tuned: six canonical shapes cover
most real canopies (from `dladgen()`'s own documentation, `TypeLidf = 1`):
...
`get.marmit.rsoil()` turns a dry reference soil spectrum into a wet one
(Tutorial 03's soil-brightness section, Tutorial 16 in full):
...
`SPART()`/`spart_toa()`-family functions (Tutorial 03) use a different
soil parameterization (BSM, Brightness-Shape-Moisture) plus an
atmospheric-correction layer (SMAC) that plain `foursail()` doesn't need:
...
mapping <- data.frame(
  Trait = c("N", "Cab", "Car", "Anth", "Cbrown", "EWT", "LMA", "alpha",
            "Prot", "CBC", "Cs", "Cx", "fqe",
            "LIDFa/LIDFb/TypeLidf", "LAI", "hspot", "tts/tto/psi",
            "fraction_brown/diss/Cv/Zeta", "LAIu/sd/cd/h/skyl"),
  `PROSPECT-D` = c("x", "x", "x", "x", "x", "x", "x", "x", "", "", "", "", "",
                    "x", "x", "x", "x", "", ""),
  `PROSPECT-PRO` = c("x", "x", "x", "x", "x", "x", "", "x", "x", "x", "", "", "",
                      "x", "x", "x", "x", "", ""),
  `Liberty` = c("", "x", "", "", "", "x", "", "", "", "", "", "", "",
                 "x", "x", "x", "x", "", ""),
  `Fluspect-B/Cx` = c("x", "x", "x", "", "", "x", "x", "x", "", "", "x", "x", "x",
                       "x", "x", "x", "x", "", ""),
  `foursail2` = c("x", "x", "x", "x", "x", "x", "x", "x", "", "", "", "", "",
                   "x", "x", "x", "x", "x", ""),
  `inform` = c("x", "x", "x", "x", "x", "x", "x", "x", "", "", "", "", "",
                "x", "x", "x", "x", "", "x"),
  check.names = FALSE
)
knitr::kable(mapping, align = c("l", rep("c", 6)))
```
...
- **Tutorial 01/02** -- see these traits in action, one leaf model and
  ...
  models together.
- **Tutorial 03** -- SPART/BSM/SMAC soil and atmosphere parameters,
  end to end.
...
- For SCOPE's own (larger) trait set -- adding photosynthesis,
  ...
  Glossary](../../scopeinr/articles/trait-glossary.html).
