...
Reproducing the [`Apps/RTMs`](https://github.com/CCGCAM/RTM-Suite/tree/main/Apps/RTMs)
...
Same `get.spectral.convolution.gaussian()` call handles all three cases
above -- a full center+FWHM calibration sheet, centers-only (FWHM
estimated from spacing), or a bundled sensor name -- because none of them
have a measured SRF to fall back on, unlike step 5's Sentinel-2A example.
...
Points scattered tightly around the 1:1 line mean the Random Forest
recovered Cab well from just 13 Sentinel-2A bands on held-out data it
never trained on -- the whole point of the LUT-then-invert workflow.
