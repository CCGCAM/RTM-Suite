...
driver of `Actot` in SCOPE's Farquhar-type photosynthesis model, so even
across just 10 randomly-drawn LUT rows the relationship is visibly
tighter than temperature-vs-pigment would be. The right panel shows the
other half of what makes SCOPEinR more than plain PROSAIL: fluorescence
(`EoutF`) is sensitive to leaf pigments, not just to LAI/structure --
substitute `Table.LUT.many$Anth` or `Table.LUT.many$Car` for `Cab` above
to see the same fluorescence output against anthocyanin or carotenoid
content instead.
