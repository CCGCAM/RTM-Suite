...
# headings throughout the rendered HTML (e.g. a data.frame's printed values,
# or a Python traceback's own text). Wrap the default, don't replace it.
...
fourSAIL and foursail2 both ran for real, and the deliberately-wrong
canopy name failed with a clear error instead of a silent wrong result —
`simulate_RTM()` is meant to be the single entry point regardless of which
leaf/canopy combination you pick.
...
Same native 1nm spectrum, two different sensors: Sentinel-2A's 13
multispectral bands vs. PRISMA's 230+ narrow hyperspectral bands — pick
whichever matches the real sensor your own reflectance data comes from.

# Vegetation indices

`get.indices.v2()` computes the common vegetation indices directly from a
simulated or convolved spectrum -- here's a handful of them for the single
spectrum simulated above, rounded to 3 decimals for readability:
...
Each column is one index computed from the same simulated spectrum —
`get.indices.v2()` returns dozens more than the 6 shown here; drop
`[, 1:6]` to see the full set.
