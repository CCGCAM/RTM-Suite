...
- leaf.model:
  ...
  Fluspect-Cx leaf optics (`get.fluspect_mSCOPE`), required by the
  fluorescence pipeline (`RTMf`). Passing anything other than
  `'fluspect-CX'` triggers a warning. Kept as an argument for forward
  compatibility.
