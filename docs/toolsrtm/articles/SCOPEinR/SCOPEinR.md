...
The **SCOPEin** R package is designed to execute the Soil Canopy
...
al. (2009) and extended by Yang et al. (2020). The **SCOPEin**R package
enables seamless integration of the SCOPE model within the R
environment, offering users the flexibility to harness its capabilities
for in-depth analysis.

We have integrated **ToolsRTM** and **SCOPEinR** package into the online
RT-platform, and the R packages are available under the GNU license. You
can access the online RT simulator at
...
3.  Generate a correlation dataset: The getCor function from the
    **ToolsRTM** package is called to create a correlation structure for
    ...
    - `rho`rho = 0.99: Sets the correlation coefficient for the
      generated variables.
...
Total simulations: 100 Total execution time: 38.64967
...
saved. In the example above (Fig. 2), we extract extra outputs such as:
...
**Fig. 3** Directory containing all outputs generated by the
**SCOPEinR** package.
