...
specified path using `renv::dependencies()`. If `renv` is not installed,
it installs `renv` first. THis is designed for getting the packages that
the Shiny app needs in this package
