...
[`ToolsRTM::srf.prisma`](srf.prisma.md),
[`ToolsRTM::srf.sentinel2a`](srf-sentinel2.md), and
[`ToolsRTM::srf.sentinel2b`](srf-sentinel2.md) (generalizes what used to
be two separate, app-only helper functions,
`convolve_prisma()`/`convolve_smac_sensor()`, in the AEO-Course PROSAIL
Shiny app's own app.R).
...
- srf:
  ...
  that wavelength (e.g. [`ToolsRTM::srf.prisma`](srf.prisma.md),
  [`ToolsRTM::srf.sentinel2a`](srf-sentinel2.md),
  [`ToolsRTM::srf.sentinel2b`](srf-sentinel2.md)).

- fwhm:

  Optional data frame with a `fwhm` column, one row per band in `srf`
  (same order), e.g. [`ToolsRTM::fwhm.prisma`](fwhm.prisma.md) – when
  supplied, its (precisely interpolated) FWHM values are used instead of
  the coarser half-max-crossing estimate this function would otherwise
  derive directly from `srf`'s own sampled weight profile. Sentinel-2A/B
  have no equivalent bundled FWHM table, so leave this `NULL` for them.
