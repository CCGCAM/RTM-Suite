...
([`get.sobol.indices`](get.sobol.indices.md)'s `I.Johnson_norm` column,
via
[`sensitivity::johnson()`](https://rdrr.io/pkg/sensitivity/man/johnson.html))
at each wavelength – i.e. how much each trait relatively contributes to
explaining reflectance variance at that wavelength. Produces the data
behind a stacked "relative contribution (percent) vs wavelength" plot
(traits stacked to 100 percent at every wavelength). Despite the name,
this does NOT use [`get.sobol.indices()`](get.sobol.indices.md)'s own
`STi` (total Sobol index) column – that column has a known bug (see
`ToolsRTM/R/get.sobol.indices.R` and this function's own source
comments) and produces a near-uniform, physically-meaningless split
across traits; `I.Johnson_norm` is the same metric this package's
reference sensitivity scripts (`TOcheck/sensibilidad/`) already use for
this exact figure.
...
- distribution:
  ...
  is drawn from (bounds/mean+sd come from
  [`ToolsRTM::inputsPROSAIL`](rtm-input-specs.md)).
...
A data.frame with columns `wavelength`, `trait`, and `STi_pct` (Johnson
relative-importance index, normalized to sum to 100% across traits at
each wavelength – the column name is kept as `STi_pct` for backward
compatibility even though it is no longer a Sobol total index, see the
source comments) – long format, ready for
