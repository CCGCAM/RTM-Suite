...
getMLmodel is a function for retrain a deep model with a prefixed
configuration
...
- n.neurons:
  ...
  number of neurons to use by the the model. By default is 128 (only for
  hidden layer model is implemented)

- n.layers:

  number of hidden-layer to use by the the model. By default is 4 (only
  for hidden layer model is implemented))
